...
def main():
    ...
    # max height
    print("Altura Máxima = %.5f" % maxHeight(v0, launchAngle, g));
    
    # max range
    print("Alcance Máximo = %.2f" % maxRange(v0x, t));

    # velocity before touching the ground
    print("vxToque = %.2f" % vxTouch(v0, launchAngle));
    print("vyToque = %.2f" % vyTouch(v0, launchAngle, g, t));
    print("vToque = %.2f" % velocityModuleTouch(vxTouch(v0, launchAngle), vyTouch(v0, launchAngle, g, t)));
    
    # when the projectile is at max height
    print("vxAlturaMax = %.2f" % vxMaxHeight(v0, launchAngle));
    print("vyAlturaMax = %.2f" % vyMaxHeight(v0, launchAngle, g, t));
    print("vAlturaMax = %.2f" % vMaxHeight(vxMaxHeight(v0, launchAngle), vyMaxHeight(v0, launchAngle, g, t)));
    ...
    tcm = 1.8;
    vm = 0.766;
    print("x milada = %.3f" % chirpyMilada(tcm,vm));
